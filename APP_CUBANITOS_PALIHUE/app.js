...
const DEFAULT_ADMIN_EMAIL = String(APP_CONFIG.ADMIN_EMAIL || "admin@tu-negocio.com");
const LOCKED_DEMO_MODE = APP_CONFIG.LOCKED_DEMO_MODE === true;
const SHOW_ADMIN_EMAIL = APP_CONFIG.SHOW_ADMIN_EMAIL !== false;
const BASE_STORAGE_PREFIX = String(APP_CONFIG.STORAGE_PREFIX || "cliente_demo")
  .toLowerCase()
  .replace(/[^a-z0-9_]/g, "_");
const ACTIVE_DEMO_GLOBAL_KEY = `${BASE_STORAGE_PREFIX}_active_demo_code`;
const normalizeAccessCode = (value) => String(value || "").trim().toUpperCase();
const configuredDefaultDemoCode = normalizeAccessCode(APP_CONFIG.DEFAULT_DEMO_CODE || "DEMO");
let activeDemoCode = configuredDefaultDemoCode;
try {
  activeDemoCode = normalizeAccessCode(localStorage.getItem(ACTIVE_DEMO_GLOBAL_KEY)) || configuredDefaultDemoCode;
} catch {}
if (LOCKED_DEMO_MODE) activeDemoCode = configuredDefaultDemoCode;
const STORAGE_PREFIX = `${BASE_STORAGE_PREFIX}_${activeDemoCode.toLowerCase().replace(/[^a-z0-9_]/g, "_")}`;
const storageKey = (suffix) => `${STORAGE_PREFIX}_${suffix}`;
let activeDemo = {
  id: String(APP_CONFIG.DEFAULT_DEMO_ID || "demo"),
  code: activeDemoCode,
  name: String(APP_CONFIG.APP_NAME || "Carrito Demo"),
  app_name: String(APP_CONFIG.APP_NAME || "Carrito Demo"),
  footer_text: String(APP_CONFIG.FOOTER_TEXT || "Carrito Demo - App de Caja"),
  logo_url: String(APP_CONFIG.LOGO_URL || "logo.png"),
};
...
const DB_ONLY_MODE = false; // permite usar la demo aunque Supabase este pausado
const STRICT_CLOUD_SYNC = false; // si falla Supabase, guarda local y sincroniza al volver
const DISABLE_LOCAL_DATA_CACHE = false; // mantiene datos locales de la demo como respaldo
const ENABLE_PEYA = false;
...
const authCodeLabelEl = $("#auth-code-label");
const authEmailEl = $("#auth-email");
const authEmailFieldEl = $("#auth-email-field");
...
function applyDemoBrand() {
  const appName = String(activeDemo?.app_name || activeDemo?.name || APP_CONFIG.APP_NAME || "Mi Carrito");
  const footerText = String(activeDemo?.footer_text || APP_CONFIG.FOOTER_TEXT || `${appName} - App de Caja`);
  const logoUrl = String(activeDemo?.logo_url || APP_CONFIG.LOGO_URL || "logo.png");
...
function applyAuthFormMode() {
  if (authCodeLabelEl) authCodeLabelEl.textContent = LOCKED_DEMO_MODE ? "Código admin" : "Código del carro o clave admin";
  if (authCodeEl) authCodeEl.placeholder = LOCKED_DEMO_MODE ? "Ingresá el código admin" : "Ingresá el código";
  if (authEmailFieldEl) authEmailFieldEl.classList.toggle("hidden", !SHOW_ADMIN_EMAIL);
  if (authEmailEl && !String(authEmailEl.value || "").trim()) authEmailEl.value = DEFAULT_ADMIN_EMAIL;
}

function fallbackDemoByCode(code) {
  const normalized = normalizeAccessCode(code);
  const demos = Array.isArray(APP_CONFIG.DEMOS) ? APP_CONFIG.DEMOS : [];
  return demos.find((demo) => normalizeAccessCode(demo.code || demo.access_code || demo.id) === normalized) || null;
}

async function resolveDemoByCode(code) {
  const normalized = normalizeAccessCode(code);
  if (!normalized) return null;

  if (hasSupabaseClient()) {
    const { data, error } = await window.supabase.rpc("resolve_demo_by_code", { p_code: normalized });
    if (error) {
      const msg = String(error.message || "").toLowerCase();
      if (!msg.includes("could not find the function") && !msg.includes("schema cache")) throw error;
    }
    const row = Array.isArray(data) ? data[0] : data;
    if (row?.id) return {
      id: String(row.id),
      code: normalized,
      name: String(row.name || row.app_name || normalized),
      app_name: String(row.app_name || row.name || normalized),
      footer_text: String(row.footer_text || ""),
      logo_url: String(row.logo_url || ""),
    };
  }

  const fallback = fallbackDemoByCode(normalized);
  if (!fallback) return null;
  return {
    id: String(fallback.id || normalized.toLowerCase()),
    code: normalized,
    name: String(fallback.name || fallback.app_name || normalized),
    app_name: String(fallback.app_name || fallback.name || normalized),
    footer_text: String(fallback.footer_text || APP_CONFIG.FOOTER_TEXT || ""),
    logo_url: String(fallback.logo_url || APP_CONFIG.LOGO_URL || "logo.png"),
  };
}

async function ensureActiveDemo() {
  try {
    const demo = await resolveDemoByCode(activeDemoCode);
...
function getSalePaymentLabel(sale) {
  ...
  const peya = ENABLE_PEYA ? Number(sale?.totals?.peya || 0) : 0;
  ...
  if (ENABLE_PEYA && peya > 0) return `PeYa ($${money(peya)})`;
  if (transfer > 0) return `Transferencia ($${money(transfer)})`;
  if (ENABLE_PEYA && channel === "pedidosya" && total > 0) return `PeYa ($${money(total)})`;
...
function getVentasSplit(sale) {
  ...
  if (ENABLE_PEYA && peya > 0) return { cash: 0, transfer: 0, peya };
  if (transfer > 0) return { cash: 0, transfer, peya: 0 };
  if (ENABLE_PEYA && channel === "pedidosya" && total > 0) return { cash: 0, transfer: 0, peya: total };
...
function sanitizeProviderList(list) {
  const banned = new Set(["GARRAFAS", "PEDIDO YA", "PEDIDOSYA", "PEYA"]);
  return Array.from(
    new Set(
      list
...
function initDeferredUi() {
  ...
  if (ENABLE_PEYA) initPeyaLiquidationRangePicker();
  initInfoRangePicker();
...
function buildHourlyStats(list) {
  ...
  for (const s of list || []) {
    ...
    row.count += 1;
    row.total += total;
    if (ENABLE_PEYA && String(s?.channel || "presencial") === "pedidosya") row.pedidosya += total;
...
function summarizeChannelVs(list) {
  ...
  for (const s of list || []) {
    ...
    if (hour == null || hour < INFO_STATS_START_HOUR || hour > INFO_STATS_END_HOUR) continue;
    const total = Number(s?.totals?.total || 0);
    if (ENABLE_PEYA && String(s?.channel || "presencial") === "pedidosya") pedidosya += total;
    else presencial += total;
  }
  let winner = "Empate";
  if (presencial > pedidosya) winner = "Presencial";
  if (ENABLE_PEYA && pedidosya > presencial) winner = "PedidosYa";
  ...
  if (mode === "day") {
    ...
    infoStatsSummaryEl.innerHTML = `
      ...
      <div class="kpi kpiWide">
        <div class="kpi-title">Presencial</div>
        <div class="kpi-value">$${money(channel.presencial)}</div>
      </div>
    `;
...
async function upsertCarryoverToDB(month, values) {
  ...
  const { error } = await window.supabase
    .from("monthly_carryovers")
    .upsert(payload, { onConflict: "demo_id,month" });
  if (error) throw error;
}
...
async function insertSaleToDB(sale) {
  ...
  if (String(error.message || "").toLowerCase().includes("channel")) {
    throw new Error("Falta la columna channel en sales. Actualiza la tabla en Supabase.");
  }
  throw error;
}
...
async function updateSaleInDB(sale) {
  ...
  let { error } = await window.supabase.from("sales").update(payload).eq("demo_id", activeDemo.id).eq("id", sale.id);
  if (!error) return;
  if (String(error.message || "").toLowerCase().includes("peya")) {
    const { peya, ...fallback } = payload;
    const retry = await window.supabase.from("sales").update(fallback).eq("demo_id", activeDemo.id).eq("id", sale.id);
    if (!retry.error) return;
    error = retry.error;
  }
  throw error;
...
async function updateExpenseInDB(expense) {
  ...
  for (const base of variants) {
    ...
    for (const desc of candidates) {
      const attemptPayload = { ...base, description: desc };
      const { error } = await window.supabase.from("expenses").update(attemptPayload).eq("demo_id", activeDemo.id).eq("id", expense.id);
      if (!error) return;
      lastError = error;
      const msg = String(error.message || "").toLowerCase();
      const canRetryLen = msg.includes("too long") || msg.includes("value too long") || msg.includes("character varying");
...
async function deleteSaleById(id) {
  if (!hasSupabaseClient()) throw new Error("Sin internet.");
  if (!session?.user || !isAdmin) throw new Error("Solo admin");
  const { error } = await window.supabase.from("sales").delete().eq("demo_id", activeDemo.id).eq("id", id);
  if (error) throw error;
...
async function deleteDaySales(dayKey) {
  if (!hasSupabaseClient()) throw new Error("Sin internet.");
  if (!session?.user || !isAdmin) throw new Error("Solo admin");
  const { error } = await window.supabase.from("sales").delete().eq("demo_id", activeDemo.id).eq("day", dayKey);
  if (error) throw error;
...
async function deleteExpenseById(id) {
  if (!hasSupabaseClient()) throw new Error("Sin internet.");
  if (!session?.user || !isAdmin) throw new Error("Solo admin");
  const { error } = await window.supabase.from("expenses").delete().eq("demo_id", activeDemo.id).eq("id", id);
  if (error) throw error;
...
async function refreshSession() {
  ...
  if (forceGuestMode) {
    ...
    const adminEmailSession = String(remoteSession?.user?.email || "").toLowerCase() === String(DEFAULT_ADMIN_EMAIL).toLowerCase();
...
async function applyAuthState() {
  if (!hasSupabaseClient()) {
    ...
    if (!forceGuestMode && rememberedAdmin) {
      session = { user: { id: "offline-admin", email: DEFAULT_ADMIN_EMAIL } };
      isAdmin = true;
    } else {
      session = null;
      isAdmin = false;
...
function applyAuthUi() {
  const demoLabel = String(activeDemo?.app_name || activeDemo?.name || activeDemoCode || "Demo");
  if (authUserEl) authUserEl.textContent = session?.user ? `Usuario: ${session.user.email}` : "";

  if (!session?.user) {
    setBadge("Invitado", "bad");
    setAuthMsg(`${demoLabel}. Invitado: podes guardar ventas. Gastos y edicion solo admin.`);
  ...
  if (!isAdmin) {
    ...
    setAuthMsg(`${demoLabel}. Usuario no admin: podes guardar ventas. Gastos y edicion solo admin.`);
    setEditEnabled(false);
    applyCajaAccessUi();
    return;
  }
  ...
  setAuthMsg(`${demoLabel}. Admin OK. Podes guardar ventas y editar catalogo.`);
  if (forceGuestMode) {
    forceGuestMode = false;
    try { localStorage.removeItem(FORCE_GUEST_KEY); } catch {}
  }
...
btnLoginCode?.addEventListener("click", async () => {
  try {
    if (!hasSupabaseClient()) {
      const localCode = normalizeAccessCode(authCodeEl?.value || "");
      const localDemo = LOCKED_DEMO_MODE ? null : await resolveDemoByCode(localCode);
      if (!LOCKED_DEMO_MODE && localDemo) {
        try { localStorage.setItem(ACTIVE_DEMO_GLOBAL_KEY, localDemo.code); } catch {}
        setAuthMsg(`Abriendo ${localDemo.app_name || localDemo.name}...`);
        window.location.reload();
        return;
      }
      setBadge("Sin internet", "bad");
      setAuthMsg("Sin internet: no se puede validar el codigo.");
      return;
    }
    setAuthMsg("Validando codigo...");
    const rawCode = String(authCodeEl?.value || "").trim();
    const code = normalizeAccessCode(rawCode);
    if (!code) return setAuthMsg("Ingresa un codigo.");

    const demo = LOCKED_DEMO_MODE ? null : await resolveDemoByCode(code);
    if (!LOCKED_DEMO_MODE && demo) {
      try { localStorage.setItem(ACTIVE_DEMO_GLOBAL_KEY, demo.code); } catch {}
      setAuthMsg(`Abriendo ${demo.app_name || demo.name}...`);
      window.location.reload();
      return;
    }
    ...
    const loginPromise = window.supabase.auth.signInWithPassword({
      email: String(authEmailEl?.value || DEFAULT_ADMIN_EMAIL).trim(),
      password: rawCode,
    });
...
function applyPedidosYaTheme() {
  if (!ENABLE_PEYA) {
    document.body.classList.remove("pedidosya-mode");
    return;
  }
...
function syncPayModeByChannel() {
  ...
  if ((!ENABLE_PEYA || !showPeya) && getPayMode() === "peya") {
...
$("#btn-save")?.addEventListener("click", async () => {
  ...
  if (!ENABLE_PEYA && mode === "peya") return (saveMsgEl.textContent = "Metodo de pago no disponible.");
  if (ENABLE_PEYA && mode === "peya" && activeChannel !== "pedidosya") return (saveMsgEl.textContent = "PeYa solo esta disponible en PedidosYa.");
  ...
  try {
    ...
    if (!STRICT_CLOUD_SYNC) {
      sales = [...sales, sale];
      saveListCache(LS_SALES_KEY, sales);
      enqueueOffline({ kind: "sale", op: "insert", payload: sale });
      clearActiveCart();
      salesTodayExpanded = false;
      renderAll();
      saveMsgEl.textContent = `Venta guardada localmente (${formatDayKey(saleDayKey)}). Se sincroniza cuando vuelva Supabase.`;
    } else {
...
function renderSaleCard(s) {
  ...
  const channelTag = ENABLE_PEYA && s.channel ? ` · ${s.channel === "pedidosya" ? "PedidosYa" : "Presencial"}` : "";
...
function renderCaja() {
  if (!kpiTotalEl || !kpiCashEl || !kpiTransferEl) return;
  const day = todayKey();
  ...
  for (const s of list) {
    ...
    peya += ENABLE_PEYA ? Number(s?.totals?.peya || 0) : 0;
  }
...
function renderMonthlySales() {
  if (!salesMonthInputEl || !monthTotalEl || !monthCashEl || !monthTransferEl) return;
  ...
  if (monthPeyaEl) monthPeyaEl.textContent = `$${money(peya)}`;
...
function calcCajaMonthlyData(month) {
  ...
  for (const e of expenses) {
    ...
    peyaExpenses += ENABLE_PEYA ? Number(split.peya || 0) : 0;
  }

  const carry = carryoverByMonth[month] || { cash: 0, transfer: 0, peya: 0 };
  const peyaLiqAmount = ENABLE_PEYA ? peyaLiquidations
  ...
  const peya = ENABLE_PEYA ? Number(carry.peya || 0) - peyaExpenses + peyaLiqAmount : 0;
  const total = cash + transfer + peya;
...
function buildCajaMonthLedger(month) {
  ...
  if (Number(carry.cash || 0) || Number(carry.transfer || 0) || (ENABLE_PEYA && Number(carry.peya || 0))) {
    entries.push({
      kind: "carryover",
      ...
      delta: {
        ...
        peya: ENABLE_PEYA ? Number(carry.peya || 0) : 0,
      },
    });
  }
  ...
  for (const s of sales) {
    ...
    entries.push({
      ...
      title: `Venta ${formatDayKey(dayKey)} ${String(s.time || "").trim()}`,
      delta: {
        cash: Number(s?.totals?.cash || 0),
        transfer: Number(s?.totals?.transfer || 0),
  ...
  for (const e of expenses) {
    ...
    entries.push({
      ...
      delta: {
        cash: -Number(split.cash || 0),
        transfer: -Number(split.transfer || 0),
        peya: ENABLE_PEYA ? -Number(split.peya || 0) : 0,
      },
    });
  }

  if (ENABLE_PEYA) for (const liq of peyaLiquidations) {
...
function renderCajaMonthly() {
  if (!cajaMonthInputEl || !cajaMonthTotalEl || !cajaMonthCashEl || !cajaMonthTransferEl) return;
  const month = String(cajaMonthInputEl.value || monthKeyNow());
  ...
  if (cajaMonthPeyaEl) cajaMonthPeyaEl.textContent = `$${money(snap.peya)}`;
...
function syncCarryoverInputs() {
  ...
  if (carryoverPeyaEl) carryoverPeyaEl.value = String(ENABLE_PEYA ? Number(carry.peya || 0) : 0);
}
...
function renderHistory() {
  ...
  if (!dayKeys.length) {
    historyListEl.innerHTML = `<div class="historyRow"><div><div><strong>${formatDayKey(todayKey())}</strong></div><div class="historyMeta">0 venta(s) · Efectivo $0 · Transf $0</div></div><div><strong>$0</strong></div></div>`;
...
function calcDayForTemplate(daySales) {
  ...
  for (const s of daySales) {
    ...
    if (ENABLE_PEYA && channel === "pedidosya") {
...
function calcMonthForTemplate(year, month) {
  ...
  for (const [dayKey, daySales] of byDay.entries()) {
    ...
    monthly.pedidosya += ENABLE_PEYA ? d.pedidosya : 0;
    monthly.total += d.ventasPresencial;
...
function expenseSplitPayments(e) {
  ...
  const peya = ENABLE_PEYA ? Number(e.pay_peya || 0) : 0;
  ...
  if (ENABLE_PEYA && method === "peya") return { cash: 0, transfer: 0, peya: amount };
  return { cash, transfer, peya };
}
...
$("#btn-export")?.addEventListener("click", async () => {
  ...
  try {
    ...
    rows.push([]);
    rows.push(["DIA", "COMUNES", "BAÑADOS", "OTRO", "EFECTIVO", "TRANSFERENCIA", "VENTAS", "TOTAL DIA"]);
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const totalDia = v.ventasPresencial;
      rows.push([
        dayKey,
        v.comunes,
        v.banados,
...
function normalizeExpenseMethod(method) {
  ...
  if (raw === "peya" || raw === "pedidosya" || raw === "pedidoya" || raw === "wallet") return ENABLE_PEYA ? "peya" : "transferencia";
...
function renderExpenseMixedDiff() {
  ...
  const peya = ENABLE_PEYA ? parseNum(expensePayPeyaEl?.value) : 0;
...
function renderExpenses() {
  ...
  const renderExpenseCard = (e) => `
    <div class="sale" data-expense-id="${e.id}">
      ...
      <div class="sale-items">${e.description}${
        e.method === "mixto" ? ` · Mix: Ef $${money(e.pay_cash)} / Tr $${money(e.pay_transfer)}` : ""
      }</div>
      <div class="actions" style="margin-top:8px;">
  ...
  for (const e of monthExpenses) {
    ...
    monthPeya += ENABLE_PEYA ? Number(split.peya || 0) : 0;
  }
...
document.addEventListener("click", async (e) => {
  ...
  if (editSaleBtn) {
    ...
    const defaultMethod = Number(sale.totals?.cash || 0) > 0 && Number(sale.totals?.transfer || 0) > 0
      ? "mixto"
      : Number(sale.totals?.cash || 0) > 0
      ? "efectivo"
      : "transferencia";
    const payMethodRaw = prompt("Metodo de pago (efectivo/transferencia/mixto):", defaultMethod);
    if (payMethodRaw == null) return;
    const payMethod = String(payMethodRaw).trim().toLowerCase();
    if (!["efectivo", "transferencia", "mixto"].includes(payMethod)) return alert("Metodo invalido.");
...
btnExpenseSave?.addEventListener("click", async () => {
  ...
  const payPeya = ENABLE_PEYA ? Math.max(0, parseNum(expensePayPeyaEl?.value)) : 0;
  ...
  if (method === "mixto") {
    ...
    if (Math.abs(sum - amount) > 0.01) return setExpenseMsg("En mixto, efectivo + transferencia debe dar el monto total.");
  }
  ...
  const expense = {
    ...
    pay_peya: ENABLE_PEYA && method === "mixto" ? payPeya : ENABLE_PEYA && method === "peya" ? amount : 0,
  };
  ...
  try {
    ...
    if (!STRICT_CLOUD_SYNC) {
      ...
      saveListCache(LS_EXPENSES_KEY, expenses);
      enqueueOffline({ kind: "expense", op: "insert", payload: expense });
      renderAll();
      setExpenseMsg(`Gasto guardado localmente. Se sincroniza cuando vuelva Supabase.${descriptionTrimmed ? " Descripcion resumida." : ""}`);
      resetExpenseForm();
      if (expenseFormWrapEl) expenseFormWrapEl.classList.remove("hidden");
    } else {
...
cajaMonthInputEl?.addEventListener("change", () => {
  cajaMonthHistoryExpanded = false;
  syncCarryoverInputs();
  if (ENABLE_PEYA) syncPeyaLiqInputs();
  renderCajaMonthly();
  renderCarryoverHistory();
  if (ENABLE_PEYA) renderPeyaLiqHistory();
});
btnCarryoverSaveEl?.addEventListener("click", async () => {
  ...
  const peya = ENABLE_PEYA ? Math.max(0, Number(carryoverPeyaEl?.value || 0)) : 0;
  const row = {
    id: `${Date.now()}_${Math.random().toString(16).slice(2)}`,
    month,
    cash,
